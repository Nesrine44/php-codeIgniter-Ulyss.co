...
<section class="pg-ccm pg-dcc">
    <div class="container">
        <form id="f_linkedin" action="<?php echo base_url(); ?>auth/getLinkedinInfos2/" method="POST" enctype="multipart/form-data">
            ...
            <?php foreach ($profileExperiences

            as $key => $profileExperience) { ?>
            ...
            <input type="hidden" name="experience[<?php echo $key; ?>][company][region]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                'geographicArea'); ?>"/> <input type="hidden" name="experience[<?php echo $key; ?>][company][pays]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                'country'); ?>"/> <input type="hidden" name="experience[<?php echo $key; ?>][company][effectif]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                'employeeCountRange'); ?>"/>
            ...
            <input type="hidden" name="experience[<?php echo $key; ?>][company][industry]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                'industry'); ?>"/> <input type="hidden" name="experience[<?php echo $key; ?>][company][logo]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                'logo'); ?>"/>
                        ...
                        <div class="row mgb30">
                            ...
                            <div class="col-sm-4 col-xs-12 select_field">
                                ...
                                <select <?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                                    ...
                                    <?php foreach ($secteur_activites as $secteur): ?>
                                        <option <?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                                            'industry') == $secteur->nom ? 'selected' : ''; ?> value="<?php echo $secteur->id; ?>"><?php echo $secteur->nom; ?></option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                        </div>
    ...
    <div class="row mgb_30 btn_add_cours text-right">
        ...
        <div class="col-md-12 mgb_30">
            <button type="submit" onclick="sendForm('f_linkedin'); return false;">Validez vos expériences<i class="fa fa-angle-right"></i></button>
        </div>
    </div>
    </form></div>
</section>