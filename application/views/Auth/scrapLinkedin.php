...
<section class="pg-ccm pg-dcc">
    <div class="container">
        <form id="f_linkedin" action="<?php echo base_url(); ?>auth/getLinkedinInfos/" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
            <div class="row text-center mgb30">
                <div class="col-md-12">
                    <h1 class="title_st">Validez vos expériences</h1>
                </div>
            </div>
            <div class="row mgb30">
                <div class="col-md-12">
            ...
            <?php if (is_array($profileExperiences)) { ?>
                <?php foreach ($profileExperiences as $key => $profileExperience) { ?>
                    ...
                    <input type="hidden" name="experience[<?php echo $key; ?>][company][site]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'], 'page'); ?>"/>
                    <input type="hidden" name="experience[<?php echo $key; ?>][company][ville]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'], 'city'); ?>"/>
                    <input type="hidden" name="experience[<?php echo $key; ?>][company][region]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                        'geographicArea'); ?>"/>
                    <input type="hidden" name="experience[<?php echo $key; ?>][company][pays]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'], 'country'); ?>"/>
                    <input type="hidden" name="experience[<?php echo $key; ?>][company][effectif]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'],
                        'employeeCountRange'); ?>"/>
                    ...
                    <input type="hidden" name="experience[<?php echo $key; ?>][company][city]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'], 'cityExp'); ?>"/>
                    <input type="hidden" name="experience[<?php echo $key; ?>][company][alias]" value="<?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'], 'alias'); ?>"/>
                    <div class="row profileexp">
                        <div class="col-sm-9">
                            <div class="row mgb_15">
                                ...
                                <?php
                                if (isset($profileExperience['entrepriseChoix']) && $profileExperience['entrepriseChoix'] != null) { ?>

                                    <div class="row mgb30 rowdepartement">
                                        <div class="col-sm-4 col-xs-12">
                                            <label class="cl_blue"> Choisir un nom d'entreprise :</label>
                                        </div>
                                        <div class="col-sm-4 col-xs-12 select_field">
                                            <select name="experience[<?php echo $key; ?>][entreprise_id]" class="required entreprise" id="<?php echo $key; ?>">
                                                <option value="">entreprise(s)</option>
                                                <?php foreach ($profileExperience['entrepriseChoix'] as $ent): ?>
                                                    <option value="<?php echo $ent->id; ?>"<?php echo $ModelTalentExperience != null && $ModelTalentExperience->entreprise_id == $ent->id ? 'selected' : ''; ?>> <?php echo $ent->nom; ?></option>
                                                <?php endforeach ?>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <p class="col-md-12 ">
                                            <?php if (isset($profileExperience['period'])) { ?>
                                                ...
                                                <?php } ?>
                                            <?php } ?>
                                            <br/><?php echo $LinkedinScraping->getValueInArray($profileExperience['compagny'], 'cityExp'); ?>
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <?php } ?>
            <?php } ?>
            <?php if (is_array($profileCompetences)) { ?>
                <div class="row mgb30">
                    <div class="col-md-12">
                        <div class="row mgb_15">
            ...
            <div class="row mgb_30 btn_add_cours text-right">
                ...
                <div class="col-md-12 mgb_30">
                    <button type="submit" onclick="sendForm('f_linkedin'); return true;">Validez vos expériences<i class="fa fa-angle-right"></i></button>
                </div>
            </div>
        </form>
    </div>
